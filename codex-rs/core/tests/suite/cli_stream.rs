...
async fn responses_mode_stream_cli() {
    ...
    let bin = codex_utils_cargo_bin::cargo_bin("kodex").unwrap();
...
/// Verify that `kodex exec --profile ...` preserves the active profile when it
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("kodex").unwrap();

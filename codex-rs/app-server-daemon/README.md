...
`codex-app-server-daemon` backs the machine-readable `kodex app-server`
...
```sh
$HOME/.codex/packages/standalone/current/kodex app-server daemon bootstrap --remote-control
```
...
pidfile-backed detached process, and reports `auto_update_enabled: false`.
The detached updater loop is disabled in this fork.

## Installation and update cases

The daemon assumes Kodex is installed through the managed standalone path and always launches
the standalone managed binary under `CODEX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| Managed binary is present, but only `start` is used | `start` uses `CODEX_HOME/packages/standalone/current/kodex` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| Managed binary is present, then `bootstrap` is used | The pidfile backend uses `CODEX_HOME/packages/standalone/current/kodex` | No | No. Bootstrap starts app-server and does not launch a detached updater loop in this fork. |
...
For managed standalone installs:

- lifecycle commands always use the standalone managed binary path
- `bootstrap` is supported
- `bootstrap` does not start a detached updater loop in this fork
- updating the managed binary path requires an out-of-band installer or package
  manager followed by an explicit `restart`
...
- with `bootstrap`, no detached updater loop is running in this fork; a
  currently running app-server remains on the old executable image until an
  explicit `restart`
...
Top-level `kodex remote-control` bootstraps with `--remote-control` when the
managed app-server is not running. Otherwise it enables remote control and
...
- `app-server-updater.pid` is retained for compatibility but unused while

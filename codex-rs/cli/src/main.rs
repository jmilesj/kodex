...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `kodex mcp`: `kodex`, `kodex exec`, `kodex review`, `kodex resume`, `kodex archive`, `kodex delete`, `kodex unarchive`, `kodex fork`, `kodex mcp`, `kodex sandbox`, and `kodex debug prompt-input`."
        ),
    }
}
...
async fn load_exec_server_remote_auth_provider(
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `kodex login` or set CODEX_API_KEY",
    )
    .await?;
...
mod tests {
    ...
    fn sandbox_full_auto_no_longer_parses() {
        let result = MultitoolCli::try_parse_from(["kodex", "sandbox", "--full-auto", "--"]);
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run kodex resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }

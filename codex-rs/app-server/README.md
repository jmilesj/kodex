...
`kodex app-server` is the interface Codex uses to power rich interfaces such as the [Codex VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `kodex app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).
...
The unix socket transport is intended for local app-server control-plane clients. `kodex app-server proxy`
...
Currently, you can dump a TypeScript version of the schema using `kodex app-server generate-ts`, or a JSON Schema bundle via `kodex app-server generate-json-schema`. Each output is specific to the version of Codex you used to run the command, so the generated artifacts are guaranteed to match that version.

```
kodex app-server generate-ts --out DIR
kodex app-server generate-json-schema --out DIR
```
...
Applications building on top of `kodex app-server` should identify themselves via the `clientInfo` parameter.
...
Use `thread/goal/set` to create or update the current goal for a materialized thread. Clients can set `budgetLimited` when they stop because a token budget is exhausted or nearly exhausted, and `blocked` when progress is waiting on outside intervention. The system also sets `budgetLimited` when accounting crosses a configured token budget.
...
`kodex app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
kodex app-server generate-ts --out DIR --experimental
kodex app-server generate-json-schema --out DIR --experimental
```

...
"#
    ...
    let codex = if let Ok(path) = codex_utils_cargo_bin::cargo_bin("kodex") {
        path
    } else {
        ...
        if fallback.is_file() {
            ...
            eprintln!("skipping integration test because kodex binary is unavailable");
            return Ok(());
        }
    };
    ...
    let exec_output = std::process::Command::new(&codex)
        ...
        .output()
        .context("failed to execute kodex exec")?;
    anyhow::ensure!(
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for kodex resume to exit");
        }
    };
    ...
    anyhow::ensure!(
        ...
        "unexpected exit code from kodex resume: {exit_code}; output: {output_text}",
    );

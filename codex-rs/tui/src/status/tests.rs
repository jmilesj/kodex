...
async fn test_config(temp_home: &TempDir) -> Config {
    test_config_with_fallback_cwd(temp_home, None).await
}

async fn test_config_with_fallback_cwd(
    temp_home: &TempDir,
...
async fn status_snapshot_shows_chatgpt_plan_without_email() {
    let temp_home = TempDir::new().expect("temp home");
    let isolated_cwd = TempDir::new().expect("isolated cwd");
    write_models_cache(temp_home.path()).expect("write models cache");
    let mut config =
        test_config_with_fallback_cwd(&temp_home, Some(isolated_cwd.path().to_path_buf())).await;
    config.model = Some("gpt-5.1-codex-max".to_string());
    config.model_provider_id = "openai".to_string();

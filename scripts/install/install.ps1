...
function Get-ReleaseAssetMetadata {
    ...
    if ($null -eq $asset) {
        throw "Could not find release asset $AssetName for Kodex $ResolvedVersion."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded Kodex archive checksum did not match release metadata. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Resolve-Version {
    ...
    if (-not $release.tag_name) {
        Write-Error "Failed to resolve the latest Kodex release version."
        exit 1
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -CodexPath (Join-Path $StandaloneCurrentDir "kodex.exe")
...
function Test-VisibleCodexCommand {
    ...
    $codexCommand = Join-Path $VisibleBinDir "kodex.exe"
    & $codexCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed Kodex command failed verification: $codexCommand --version"
    }
}
...
$packageAsset = "kodex-npm-$npmTag-$resolvedVersion.tgz"
    ...
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target)) {
            ...
            $copyMap = @{
                ...
                "kodex/codex-command-runner.exe" = "codex-resources\codex-command-runner.exe"
                "kodex/codex-windows-sandbox-setup.exe" = "codex-resources\codex-windows-sandbox-setup.exe"

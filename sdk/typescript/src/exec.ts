...
function findCodexPath(): CodexPathResolution {
  ...
  try {
    ...
    throw new Error(
      `Unable to locate Kodex CLI binaries. Ensure ${CODEX_NPM_NAME} is installed with optional dependencies.`,
    );
  }
  ...
  if (!nativePackage) {
    throw new Error(
      `Unable to locate Kodex CLI binaries for ${targetTriple}. Ensure ${CODEX_NPM_NAME} is installed with optional dependencies.`,
    );

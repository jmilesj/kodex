...
# Start `kodex exec-server` and run codex-tui.
[no-cd]
...
app-server-test-client *args:
    ...
    cargo run -p codex-app-server-test-client -- --codex-bin ./target/debug/kodex {args}
...
[unix]
bazel-kodex *args:
    bazel run //codex-rs/cli:kodex --run_under="cd $PWD &&" -- "$@"

[windows]
bazel-kodex *args:
    bazel run //codex-rs/cli:kodex --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)
